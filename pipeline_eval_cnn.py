...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        for r in range(num_rounds):
            model_path = os.path.join('exp', 'models', 'cnn', dataset, f'round-{r}')
            Path(model_path).mkdir(parents=True, exist_ok=True)
            # Path(os.path.join(eval_path, f'round-{r}')).mkdir(parents=True, exist_ok=True)
            
            classifier, time_train, acc_train, train_size = train(dataset=dataset)
            # save trained model
            print(f'[INFO] Save trained model to {model_path}')
            pickle.dump(classifier, open(os.path.join(model_path, 'cnn.pkl'), 'wb'))
            
            time_test, acc_test, test_size = eval(classifier, dataset=dataset)
            eval_rounds[0] += acc_train
            eval_rounds[1] += acc_test
            eval_rounds[2] += time_train
            eval_rounds[3] += time_test
            
            eval_scores.append(['cnn', dataset, f'round-{r}', acc_train, acc_test, time_train, time_test, train_size, test_size])
            print(f'[INFO] scores:\n{eval_scores[0]}\n{eval_scores[-1]}')
            
        eval_analysis.append(['cnn', dataset, eval_rounds[0]/num_rounds, eval_rounds[1]/num_rounds, eval_rounds[2]/num_rounds, eval_rounds[3]/num_rounds])
    ...
    report.to_csv(filepath, index=False)
    print('[INFO] Done evaluating CNN.')
    
    filename = f'eval_analysis-cnn.csv'
    filepath = os.path.join('exp/eval', filename)
    print(f'[INFO] Save evaluation analysis to {filepath}')
    report = pd.DataFrame(eval_analysis[1:], columns=eval_analysis[0])
    report.to_csv(filepath, index=False)
