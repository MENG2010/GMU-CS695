...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        for r in range(num_rounds):
            model_path = os.path.join('exp', 'models', 'rocket-cnn', dataset, f'round-{r}')
            ...
            pickle.dump(classifier, open(os.path.join(model_path, 'rocket-cnn.pkl'), 'wb'))
            ...
            eval_scores.append(['rocket-cnn', dataset, f'round-{r}', acc_train, acc_test, time_train, time_test, train_size, test_size])
            print(f'[INFO] scores:\n{eval_scores[0]}\n{eval_scores[-1]}')
            
        eval_analysis.append(['rocket-cnn', dataset, eval_rounds[0]/num_rounds, eval_rounds[1]/num_rounds, eval_rounds[2]/num_rounds, eval_rounds[3]/num_rounds])
    ...
    filename = 'eval_scores-rocket-cnn.csv'
    ...
    filename = f'eval_analysis-rocket-cnn.csv'
    filepath = os.path.join('exp/eval', filename)
    print(f'[INFO] Save evaluation analysis to {filepath}')
    report = pd.DataFrame(eval_analysis[1:], columns=eval_analysis[0])
    report.to_csv(filepath, index=False)
