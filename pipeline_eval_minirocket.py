...
def train(dataset):
    ...
    # fit a classifier
    print(f'[INFO] Training minirocket-ridge classifier on {dataset}...')
...
if __name__ == '__main__':
    ...
    for dataset in effective_datasets:
        ...
        for r in range(num_rounds):
            ...
            pickle.dump(classifier, open(os.path.join(model_path, 'minirocket-ridge.pkl'), 'wb'))
            ...
            eval_scores.append(['minirocket-ridge', dataset, f'round-{r}', acc_train, acc_test, time_train, time_test, train_size, test_size])
            print(f'[INFO] scores:\n{eval_scores[0]}\n{eval_scores[-1]}')
            
        eval_analysis.append(['minirocket-ridge', dataset, eval_rounds[0]/num_rounds, eval_rounds[1]/num_rounds, eval_rounds[2]/num_rounds, eval_rounds[3]/num_rounds])
    ...
    filename = 'eval_scores-minirocket.csv'
    ...
    report.to_csv(filepath, index=False)
    print('[INFO] Done evaluating MiniRocket-Ridge.')
    
    filename = f'eval_analysis-minirocket.csv'
    filepath = os.path.join('exp/eval', filename)
    print(f'[INFO] Save evaluation analysis to {filepath}')
