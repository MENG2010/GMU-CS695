...
report_out_dir = os.path.join(out_dir, 'results')

kernel_functions = ['linear', 'poly', 'rbf', 'sigmoid']


def optimize_params(dataset):
    pass
...
def train(dataset):
    ...
    poly_svm = Pipeline([
        StandardScaler(with_mean=False),
        SVC(kernel='poly', tol=1e-6, gamma='scale')])
    rbf_svm = Pipeline([
        StandardScaler(with_mean=False),
        SVC(kernel='rbf', tol=1e-6, gamma='scale')])
    sigmoid_svm = Pipeline([
        StandardScaler(with_mean=False),
        SVC(kernel='sigmoid', tol=1e-6, gamma='scale')])
    
    pipe_dict = {
        0: ['linear svm', linear_svm],
        1: ['poly svm', poly_svm],
        2: ['rbf svm', rbf_svm],
        3: ['sigmoid svm', sigmoid_svm]
    }
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        for r in range(num_rounds):
            ...
            pickle.dump(classifier, open(os.path.join(model_path, 'rocket-svm.pkl'), 'wb'))
            ...
            eval_scores.append(['rocket-svm', dataset, f'round-{r}', acc_train, acc_test, time_train, time_test, train_size, test_size])
            print(f'[INFO] scores:\n{eval_scores[0]}\n{eval_scores[-1]}')
            
        eval_analysis.append(['rocket-svm', dataset, eval_rounds[0]/num_rounds, eval_rounds[1]/num_rounds, eval_rounds[2]/num_rounds, eval_rounds[3]/num_rounds])
        filename = f'eval_analysis-rocket-svm-{dataset}.csv'
    ...
    filename = 'eval_scores-rocket-svm.csv'
    filepath = os.path.join(report_out_dir, filename)
    print(f'[INFO] Save evaluation scores to {filepath}')
    report = pd.DataFrame(eval_scores[1:], columns=eval_scores[0])
    report.to_csv(filepath, index=False)
    print('[INFO] Done evaluating Rocket-SVM.')
    
    filename = f'eval_analysis-rocket-svm.csv'
    filepath = os.path.join(report_out_dir, filename)
    print(f'[INFO] Save evaluation analysis to {filepath}')
    report = pd.DataFrame(eval_analysis[1:], columns=eval_analysis[0])
    report.to_csv(filepath, index=False)
